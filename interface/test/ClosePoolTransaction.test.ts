...
describe('Close Pool Transaction', () => {
  ...
  beforeEach(async () => {
    ...
    //Close User
    const closeUserTx = await closeUserTransaction(userWallet.publicKey);
    await sendAndConfirmTransaction(connection, closeUserTx, [userWallet]);
  });
